...
export default class SetupDrawer extends PureComponent {
    ...
    _renderItem({ item }) {
        ...
        return (
            <LightListItem
                key={'setupDrawer' + roleID}
                iconName={roleIcons[roleID][1]} iconType={roleIcons[roleID][0]}
                title={`${this.state.originalSetup[roleID].length} ${roleName[roleID]}`}
                subTitle={roleDescription[roleID]}
                moreTitle={roleID < 0 ? "🐺" : ""}
            />
